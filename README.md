...
1. Provide font `<span>` tags with `@class` or `@style` attributes
   according to font style keywords (`italic` / `bold` / `monospace`) generated by `extractdvipsk.lua`
...
1. Extract font style features (`italic` / `bold` / `monospace`) directly from the
   `.otf` files using `freetype` library in case of `.xdvips` maps generated by `luafonts.lua`.

1. Escape characters `<`, `>` and `&` to their `HTML` entities equivalents when used in `OpenType` fonts.
