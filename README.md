...
1. Extract font style features (`italic` / `bold` / `monospaced`) directly from the
   `.otf` files using the `freetype` library in case of `.xdvips` maps generated by `luafonts.lua`.
